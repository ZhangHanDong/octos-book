...
Ch5 已经看到，`ContextOverflow` 不再只是一个普通错误：它会进入 `LoopDecision::CompactAndRetry`，由主循环先触发 turn compaction helper，再继续下一轮。也就是说，compaction 已经从“节省 token 的优化”变成了 Agent Loop 的恢复路径之一（`crates/octos-agent/src/agent/loop_runner.rs:321-339`）。
...
`SessionSummary` 因此不是长期记忆，也不是普通聊天摘要；它是 typed compaction summary，用来在有限窗口内保留 goal、constraints、decisions、files 和 next steps（`crates/octos-agent/src/summarizer.rs`; `crates/octos-core/src/task.rs:233`）。长期记忆仍由 Ch4 的 memory 系统负责。
...
validator preservation 是这套机制的关键。`validators.rs` 里的 outcome 会以 schema version JSONL 形式持久化；required validator 的失败会阻止 terminal success，optional validator 的失败只产生 warning（`crates/octos-agent/src/validators.rs:84-123`, `crates/octos-agent/src/workspace_git.rs:662-750`）。所以压缩策略不是“尽可能短”，而是“在变短的同时保住可验证契约”。这也是后续 Ch9 讨论 Harness validator runner、Ch12 讨论 workflow artifact gate 的基础。
...
5. **contract-gated compaction**：当前主分支把 compaction 接入 `CompactAndRetry`，并通过 typed `SessionSummary`、workspace policy 和 validator ledger 保证压缩后仍能维持任务约束。
