...
> **版本演化说明**
> 本章分析基于当前 `../octos` main 分支，octos-memory crate 位于 `crates/octos-memory/src/`，按 `tokei` 统计共 1,635 行 Rust 源文件。相比早期版本，MemoryStore 的 entity bank 已经接入 `save_memory` / `recall_memory` 工具和系统提示摘要注入；EpisodeStore 也支持 embedding 后写入与 tombstone 删除。
