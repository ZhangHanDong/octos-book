...
5. **7 天窗口记忆**：自动注入近一周的经验上下文到系统提示，超出窗口的经验通过搜索检索。
...
> **版本演化说明**
> 本章分析基于 octos v0.1.0，octos-memory crate 位于 `crates/octos-memory/src/`。截至本书写作时，混合搜索的权重配置和 HNSW 参数无重大变化。MemoryStore 的实体库功能可能在后续版本中扩展。
