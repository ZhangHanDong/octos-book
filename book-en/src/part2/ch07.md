...
flowchart TB
    subgraph "Layer 1: Process Isolation"
        SB["Sandbox<br/>Bwrap / sandbox-exec / Docker"]
    end
    ...
    subgraph "Layer 4: Output Sanitization"
        SAN["Credential Redaction<br/>7 key regex patterns + tool output sanitization"]
    end
...
## 7.1 Three Sandbox Backends

Shell command execution is the most dangerous Agent capability. octos isolates command execution in a restricted environment through sandboxing (`crates/octos-agent/src/sandbox/`).

### 7.1.1 Auto-Detection and Selection

`create_sandbox()` (`sandbox/mod.rs:263-301`) automatically detects available sandbox backends by priority:

| Platform | Preferred Backend | Fallback | Last Resort |
|----------|------------------|----------|-------------|
| Linux | Bwrap | Docker | NoSandbox |
| macOS | sandbox-exec | Docker | NoSandbox |
| Windows | AppContainer | Docker | NoSandbox |

`NoSandbox` is a pass-through execution -- it provides no isolation, is only used when no other option is available, and prints a warning.

### 7.1.2 Bwrap (Linux)

Bwrap (bubblewrap) is the Flatpak project's sandboxing tool, providing lightweight isolation using Linux namespaces (`sandbox/bwrap.rs`). octos builds the bwrap command in 7 stages:

1. **Environment cleanup** (`bwrap.rs:18-21`): Remove 18 dangerous variables from `BLOCKED_ENV_VARS`
2. **Read-only system binds** (`bwrap.rs:23-28`): `/usr`, `/lib`, `/bin`, `/sbin`, `/etc` are mounted with `--ro-bind` -- the sandbox can use system tools but cannot modify them
3. **Working directory bind** (`bwrap.rs:30-32`): The user's project directory is mounted with `--bind` (read-write)
4. **Temporary filesystem** (`bwrap.rs:34-39`): `--tmpfs /tmp` creates a temporary tmpfs; any files written to /tmp disappear when the sandbox exits
5. **Network isolation** (`bwrap.rs:41-43`): `--unshare-net` creates a new network namespace (if network is disabled in configuration)
6. **Process isolation** (`bwrap.rs:45-47`): `--unshare-pid` isolates the process view + `--die-with-parent` ensures cleanup when the parent process exits
7. **Command execution** (`bwrap.rs:47-48`): Switch to the working directory and execute the command with `sh -c`

### 7.1.3 macOS sandbox-exec and SBPL Injection Defense

macOS uses `sandbox-exec` to run sandboxes, with policies written in SBPL (Seatbelt Profile Language) (`sandbox/macos.rs`). SBPL is a Lisp-style language using parenthesis-delimited expressions -- which means **parentheses in user-controllable path names are a potential injection vector**.
...
The Docker backend (`sandbox/docker.rs`) provides the strongest isolation but with the highest overhead:
...
Regardless of which backend is used, all sandboxes clean 18 dangerous environment variables (`sandbox/mod.rs:26-49`):
...
These 18 variables are defined through the `BLOCKED_ENV_VARS` constant, used uniformly across all subprocess creation scenarios including sandboxes, MCP servers, and Hooks execution.

---

## 7.2 SSRF Protection

When an Agent makes network requests through `web_fetch` or `browser` tools, SSRF (Server-Side Request Forgery) protection ensures requests don't reach internal networks (`crates/octos-agent/src/tools/ssrf.rs`).
...
Prompt injection is an attack vector unique to AI Agents. octos's prompt guard (`crates/octos-agent/src/prompt_guard.rs`) detects 5 threat categories.
...
- **Medium and above**: Replaced with `[injection-blocked:<threat-kind>]`
- **Low**: Only logged, content is not modified

Processing order is reversed (from back to front) to maintain byte offset correctness during replacements.

### 7.3.3 Known Limitations

The prompt_guard documentation (`prompt_guard.rs:10-19`) explicitly documents known bypass methods:
...
### 7.4.1 Seven Key Type Detection

`sanitize.rs` (`crates/octos-agent/src/sanitize.rs`) uses regex to detect and redact multiple credential formats:
...
Additionally, there are `DATA_URI_RE` (base64 data URIs) and `HEX_RE` (64+ character hexadecimal strings, covering SHA-256/512 and similar).
...
`sanitize_tool_output()` is applied after every tool execution, cleaning in order:
...
SafePolicy (`crates/octos-agent/src/policy.rs:47-73`) is the last application-layer defense for shell command execution.
...
SafePolicy is **one layer in the defense in depth**, not a security boundary (the documentation at `policy.rs:36-46` explicitly states this). It can be bypassed through shell metacharacters, variable expansion, and similar techniques. True isolation is provided by the sandbox layer -- SafePolicy catches dangerous commands accidentally generated by the LLM, not carefully crafted attacks by malicious actors.
...
The workspace-level `deny(unsafe_code)` (`Cargo.toml:33-34`) ensures the entire project -- 9 core crates, 130,000 lines of code -- contains no `unsafe` code. All system calls (file I/O, networking, process management) are completed through Rust's standard library safe abstractions.

### 7.6.2 SecretString

API keys are stored using the `secrecy` crate's `SecretString` type (in Provider implementations under `crates/octos-llm/src/`). `SecretString` outputs `[REDACTED]` for `Debug` and `Display`, preventing keys from being accidentally written to logs. Only an explicit call to `expose_secret()` can retrieve the raw value, making every key usage a conscious decision.

---

> ### Engineering Decision Sidebar: The Practical Significance of Workspace-Level deny(unsafe_code)
>
> `deny(unsafe_code)` is not uncommon in the Rust community, but applying it to an entire workspace (9 crates, 130,000 lines of code) is a decision worth discussing.
>
...
1. **Sandbox isolation**: Three backends (Bwrap/sandbox-exec/Docker) + 18 environment variable cleanup, providing process-level isolation for Agent command execution.
...
> This chapter's analysis is based on octos v0.1.0, with security-related code distributed across `crates/octos-agent/src/sandbox/`, `tools/ssrf.rs`, `prompt_guard.rs`, `sanitize.rs`, and `policy.rs`. As of the time of writing, sandbox backends and SSRF blocking rules have not undergone major changes. Prompt Guard detection patterns may expand as new attack methods emerge.
