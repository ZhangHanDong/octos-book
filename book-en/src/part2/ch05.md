...
> This chapter follows the current source in `../octos/crates/octos-agent/src/agent/`. Future reviews should check the interaction among `loop_runner.rs`, `loop_state.rs`, `loop_compaction.rs`, `budget.rs`, `streaming.rs`, and `harness_errors.rs`, because Agent Loop changes now concentrate around typed recovery, tiered compaction, idle progress timeout, and background delivery boundaries.
