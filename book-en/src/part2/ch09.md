...
1. `split_frontmatter()` finds the frontmatter block between the first pair of `---` delimiters (`crates/octos-agent/src/skills.rs:235-252`).
2. `fm_value()` reads simple `key: value` lines such as `name`, `description`, `requires_bins`, `requires_env`, and `always` (`crates/octos-agent/src/skills.rs:178-232,255-276`).

`available` is derived from this metadata: every command listed in `requires_bins` must exist, and every environment variable listed in `requires_env` must be present (`crates/octos-agent/src/skills.rs:196-212`).

### 9.1.2 SkillsLoader and Layered Overrides

`SkillsLoader` itself only maintains a list of skill directories; the actual priority is assembled by runtime code (`crates/octos-agent/src/skills.rs:31-176`). The current gateway path layers:
...
This layering comes from `crates/octos-cli/src/commands/gateway/gateway_runtime.rs:526-527,646-667` and `crates/octos-cli/src/commands/gateway/profile_factory.rs:538-617`. The implementation scans lower-priority directories first and higher-priority directories later, removing older same-name skills through `retain` (`crates/octos-agent/src/skills.rs:68-108`).

Configuration inheritance and local skill inheritance must be separated. Sub-accounts can inherit parent profile LLM/search/apps/email configuration, but customer-installed skills do **not** inherit from the parent account. `skills_scope.rs` scopes account skills and plugin dirs to the current account's own `data_dir/skills` (`crates/octos-cli/src/skills_scope.rs:1-38`).

### 9.1.3 XML Skill Index

`build_skills_summary()` converts the visible skill set into XML and injects it into the system prompt (`crates/octos-agent/src/skills.rs:137-154`):
...
`spawn_only` is a marker on tool entries in plugin/skill manifests (`crates/octos-agent/src/plugins/manifest.rs:98-116`). Its runtime semantics live in the registry and execution loop:

- `PluginLoader` registers these tools as `spawn_only` (`crates/octos-agent/src/plugins/loader.rs:93-113`).
- `ToolRegistry` keeps custom prompt text and task tracking state for them (`crates/octos-agent/src/tools/registry.rs:123-178`).
- When the main Agent calls a `spawn_only` tool, it does not run it synchronously. It starts a background task and immediately returns the `spawn_only_message` to the model (`crates/octos-agent/src/agent/execution.rs:105-245`).
...
When a skill package contains `spawn_only` tools, `resolve_extras()` also injects that package's `SKILL.md` as prompt fragments (`crates/octos-agent/src/plugins/extras.rs:52-61`). In subagent contexts, `clear_spawn_only()` removes the marker because the subagent is already a background context, so the tool executes like a normal tool (`crates/octos-agent/src/tools/registry.rs:136-143`).
...
The runtime hot path uses `crates/octos-agent/src/plugins/manifest.rs`:
...
Structured stdout supports more semantics than `output` / `success`, including `file_modified` and `files_to_send`. Runtime also tries to detect generated files from arguments or output text and trigger automatic file return (`crates/octos-agent/src/plugins/tool.rs:321-403`).
...
- 100MB executable size limit (`crates/octos-agent/src/plugins/loader.rs:213-224`)
- `BLOCKED_ENV_VARS` inheritance (`crates/octos-agent/src/plugins/loader.rs:273-275`, `crates/octos-agent/src/plugins/tool.rs:140-148`)
- tool-level `env` / `env_allowlist`: if explicitly listed, only those environment variables are passed; without an explicit list, legacy behavior remains, but secret-like extra env vars must be allowlisted (`crates/octos-agent/src/plugins/tool.rs:859-893`)
- `OCTOS_WORK_DIR` is injected for plugin output files (`crates/octos-agent/src/plugins/tool.rs:150-164`)
- default timeout is 600 seconds, not 30 seconds (`crates/octos-agent/src/plugins/tool.rs:35-48`)

**Risk and concurrency class**:

`risk` affects runtime approval. `high` / `critical` tools force interactive approval; if no approval bridge exists, runtime denies safely. `low` does not request approval by default, while `medium` and unknown values are surfaced but not hard-gated (`crates/octos-agent/src/plugins/manifest.rs:136-144`, `crates/octos-agent/src/plugins/tool.rs:772-820`).

`concurrency_class` recognizes `safe` and `exclusive`. Unknown values fail closed to `Exclusive`, preventing a misdeclared plugin from being executed concurrently against shared state (`crates/octos-agent/src/plugins/manifest.rs:220-263`, `crates/octos-agent/src/plugins/tool.rs:711-730`).

**Symlink rejection**: Unix executable discovery uses `symlink_metadata()` and accepts only regular files, rejecting symbolic links (`crates/octos-agent/src/plugins/loader.rs:332-340`).

### 9.2.4 Runtime PluginLoader vs `octos-plugin` SDK

Do not conflate the two layers:

**The current runtime hot path** is `crates/octos-agent/src/plugins/loader.rs`. It scans caller-provided directories, loads each subdirectory manifest, resolves extras, finds and verifies executables, creates verified copies, registers tools, and treats a single plugin failure as a warning instead of failing the entire load (`crates/octos-agent/src/plugins/loader.rs:73-140`).

**`crates/octos-plugin` is an SDK/tooling crate**, with discovery and gating abstractions:

- `discover_plugins()` scans source directories by priority and deduplicates (`crates/octos-plugin/src/discovery.rs:20-56`)
- `check_requirements()` gates by required binaries, env vars, and OS (`crates/octos-plugin/src/gating.rs:37-123`)
...
`abi_schema.rs` centralizes runtime payload schema versions for `WorkspacePolicy`, `CompactionPolicy`, `HookPayload`, `ProgressEvent`, `TaskResult`, `SessionSummary`, swarm dispatch, cost attribution, routing decisions, credential-pool config, and harness error events (`crates/octos-agent/src/abi_schema.rs:1-142`). These versions describe the serialized shapes the octos runtime can understand, not the release version of a plugin or skill.

The key rule is fail-closed compatibility. `check_supported(kind, found, supported)` returns a typed error for future versions instead of silently ignoring unknown fields (`crates/octos-agent/src/abi_schema.rs:144-186`). This lets external skills evolve with the runtime while preventing old runtimes from misreading new payloads.
...
This is not a log file. stdout remains the tool result protocol. The event sink is a structured event ABI validated as `HarnessEvent`; each event line has a size limit and is validated before being appended to JSONL (`crates/octos-agent/src/harness_events.rs:116-132`).
...
> This chapter's analysis is based on octos v0.1.0. Skills and runtime plugin code is primarily located in `crates/octos-agent/src/skills.rs`, `plugins/`, and `mcp.rs`; ecosystem discovery and gating live under `crates/octos-plugin/src/`. The MCP transport model, `spawn_only` semantics, and runtime/plugin-SDK boundary should be checked against source rather than older documentation.
