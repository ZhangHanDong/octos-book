...
> **Positioning**: This chapter dives into octos's lowest-level crate -- octos-core (currently about 8.8k lines of Rust source, mostly due to UI Protocol wire types), showing how to build the domain language for an AI Agent platform using Rust's type system. Prerequisites: Chapter 1. Applicable to all readers who want to understand octos's type foundation, especially those who want to learn Rust enum and error handling design through a real-world project (Reader A), and experienced developers who want to understand the "zero-dependency core crate" design philosophy (Reader B).

If we compare octos to a city, octos-core is its language -- not the buildings, not the roads, but the vocabulary and grammar that residents use to communicate. `Task`, `Message`, `MessageRole`, `Error`, and UI Protocol wire types define how components in the system describe their state and intent. Upper-layer octos crates depend on these types, but octos-core itself does not depend on any other crate in the workspace.

This zero-dependency constraint is not accidental. It is a deliberate architectural decision that keeps the domain language boundary clear as the system evolves: when octos-llm refactors Provider implementation or octos-bus changes channel behavior, core should absorb only truly cross-crate wire identity and domain model changes, not upper-layer implementation logic. This chapter starts with the Task state machine, analyzes these foundational types one by one, and concludes by discussing the engineering significance of this zero-dependency strategy.
...
TaskResult (`crates/octos-core/src/task.rs:125-157`) records the output of a task:
...
TokenUsage (`crates/octos-core/src/task.rs:159-173`) deserves special attention. It tracks not just input/output tokens but also `reasoning_tokens` (chain-of-thought tokens, for reasoning models like o1 and kimi-k2.5) and `cache_read_tokens`/`cache_write_tokens` (Provider cache hits/writes). These five dimensions allow upper layers to precisely calculate costs and optimize caching strategies. During serialization, zero-valued fields are skipped to avoid JSON bloat.
...
The Message definition is located at `crates/octos-core/src/types.rs:227-258`:
...
The `media` field (`types.rs:232-234`) supports multimodality: when users send images or voice, file paths are stored here, and octos-llm converts them to the appropriate Provider format (base64 encoding or URL reference) when building API requests. Empty `media` vectors are skipped during serialization.

`reasoning_content` (`types.rs:239-241`) is designed for reasoning models (such as OpenAI o1, Kimi k2.5) -- these models first output an internal reasoning process before giving a final answer. Storing reasoning content separately from the formal response lets upper layers choose whether to display the chain of thought.

`client_message_id` and `thread_id` are not Provider message-format fields. They are octos's UI/persistence protocol fields: the former correlates the client's optimistic UI bubble with the server-assigned `historySeq`; the latter tells AppUI which user turn an assistant/tool reply should render under. This is the kind of field that looks like "just metadata" at first, but in a real Agent UI it determines whether late streaming events and background tool results appear under the correct user turn.

### 2.2.2 Message Identity: ClientMessageId, ThreadId, TurnId Must Not Be Mixed

Current octos-core has split message identity into three explicit newtypes:

| Type | Source location | Meaning | Producer |
|------|-----------------|---------|----------|
| `ClientMessageId` | `types.rs:11-68` | Client-supplied optimistic UI / idempotency token | Web/AppUI client |
| `ThreadId` | `types.rs:76-178` | Render grouping key; assistant/tool replies inherit the originating user turn's thread | Usually rooted from `ClientMessageId` |
| `TurnId` | `ui_protocol.rs:269-282` | Server-side UI Protocol turn identity | Backend protocol layer |
...
The important point is not that octos added three ID strings. The important point is that these are different invariants:

- `ClientMessageId` answers "which optimistic client bubble should this persisted user message confirm?"
- `ThreadId` answers "which user turn should this assistant/tool output render under?"
- `TurnId` answers "which server-visible protocol turn is this?"

Earlier implementations could accidentally derive a reply's target from "the most recent user message." That heuristic fails once background results, tool completions, or delayed streams arrive after later user turns. The current type layer encodes the stricter rule: assistant/tool messages should carry an explicit `ThreadId`.

The constructors in `Message` show the intended usage:
...
`user_rooting_thread()` expresses the common rule directly: the user message roots a new thread, so `thread_id == client_message_id`. `assistant_with_thread()` and `tool_with_thread()` force production code to pass the originating thread explicitly, which is the structural fix for the "late result attached to the newest user bubble" class of bugs. Chapter 10 will revisit the same invariant from the message bus and persistence side.

### 2.2.3 MessageRole: Dual Implementation of as_str() and Display
...
The key lies in its two method implementations. `as_str()` (`types.rs:453-463`) returns `&'static str`:
...
The `Display` trait implementation (`types.rs:465-469`) delegates directly to `as_str()`:
...
### 2.2.4 ToolCall and Metadata Extension

ToolCall (`crates/octos-core/src/types.rs:471-479`) is the data structure used when the LLM requests a tool invocation:
...
## 2.3 Durable ABI: TaskResult and SessionSummary schema_version

`TaskResult` and `SessionSummary` are more than in-memory structs. They are durable protocol payloads: they may be written to disk, replayed by later processes, or read by a newer octos binary. That is why current octos-core gives both types an explicit `schema_version`.

In `crates/octos-core/src/task.rs:125-157`, `TaskResult` defines the durable ABI version:
...
The `serde(default = ...)` attribute is the compatibility point: JSON emitted before this field existed can still deserialize as version 1. That avoids breaking historical task results just because the durable schema became explicit later.

`SessionSummary` applies the same idea to conversation compaction (`task.rs:179-305`):
...
impl SessionSummary {
    pub fn validate_schema_version(&self)
        -> Result<(), UnsupportedSessionSummaryVersion>;
}
```
...
The asymmetric policy is intentional:

- Missing `schema_version` defaults to the current v1, so older files remain readable.
- A future `schema_version` is rejected with `UnsupportedSessionSummaryVersion`, so a current binary does not silently misinterpret a newer summary schema.

This is a useful distinction between "backward-compatible absence" and "forward-incompatible novelty." For Agent systems, silent misinterpretation is worse than a typed failure: a stale summary can change future tool choices, file edits, and user-visible answers.

---

## 2.4 Error Design: Why eyre Instead of anyhow
...
### 2.4.2 eyre vs anyhow: Selection Rationale
...
## 2.5 truncate_utf8: The UTF-8 Safety Philosophy Behind a Small Function
...
### 2.5.3 truncate_head_tail: Smart Truncation Preserving Head and Tail
...
### 2.5.4 tool_output_limit: Per-Tool Truncation Policies
...
## 2.6 SessionKey: Multi-Tenant Session Identifier Design

SessionKey (`crates/octos-core/src/types.rs:489-567`) is key to session routing in a multi-tenant system. Its design evolution reflects the expansion of requirements from single-channel to multi-channel, from single-Profile to multi-Profile, and then to topic-scoped session branches.
...
The `base_key()` method (`types.rs:525-528`) returns the part without the `#topic` suffix, used for session persistence (different topics under the same base_key share the persistence file). The `topic()` method (`types.rs:530-533`) extracts the topic suffix.

### 2.6.2 Channel Inference, Not Constructor-Time Validation

Current source does not reject unknown channels in `SessionKey::new()`; the constructor only formats the string. `is_channel_name()` (`types.rs:569-588`) is used by `split_base_key()` to decide whether the first segment of a three-part key is a `profile_id` or a `channel`. The whitelist contains 15 known channels:
...
This distinction matters: `SessionKey` provides structured parsing for routing keys, not global channel configuration validation. Channel/profile validity is checked in upper CLI/API configuration paths; core only owns the low-dependency, serializable key shape.

---

## 2.7 AgentMessage: Inter-Agent Coordination Protocol
...
> - Clear compilation boundary (octos-core is currently about 8.8k lines of Rust source, most of it UI Protocol wire types)
...
octos-core defines the entire system's domain language in about 8.8k lines of Rust source:
...
2. **Message abstraction**: Four-role unified model (System/User/Assistant/Tool) + dual `as_str()`/`Display` implementation, ensuring serialization consistency across Providers. `ClientMessageId`, `ThreadId`, and `TurnId` split client correlation, render grouping, and protocol turn identity.

3. **Durable ABI**: `TaskResult` and `SessionSummary` both carry `schema_version`. Missing versions default to v1 for backward compatibility; future summary versions fail closed with `UnsupportedSessionSummaryVersion`.

4. **Error design**: Chose eyre/color-eyre for colorized error reports and SpanTrace support. The three-layer structure (kind + context + suggestion) makes error messages actionable.

5. **UTF-8 safety utilities**: Two variants of `truncate_utf8` (in-place and copying) ensure safe truncation through `is_char_boundary()`. `truncate_head_tail` preserves head and tail information.

6. **Zero-dependency design**: The thin core strategy ensures a stable type foundation with fast compilation, supporting the independent evolution of upper-layer crates.
...
> This chapter follows the current source in `../octos/crates/octos-core/src/`. Future reviews should verify the identity newtypes, `SessionSummary` ABI, and UI Protocol capabilities in addition to Task/Message/Error.
