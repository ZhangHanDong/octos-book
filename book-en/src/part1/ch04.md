...
**EpisodeOutcome** (`episode.rs:72-81`) has four variants: `Success`, `Failure`, `Blocked`, and `Cancelled`. These correspond one-to-one with the terminal states of TaskStatus (see Chapter 2)—Episodes are created only when a Task reaches a terminal state.
...
Episodes are created and stored when a Task completes (`store.rs:87-151`). The write process:
...
This uses the simplest possible tokenization—convert to lowercase, split on non-alphanumeric characters, and filter out tokens shorter than 2 characters. For Chinese text, this effectively splits by individual characters (each Chinese character is an alphanumeric character), which is less effective than a specialized tokenizer (such as jieba), but wins on zero dependencies and zero error rate. For the use case of searching Agent experience summaries, it is sufficient.
...
| Entity bank | `bank/entities/<slug>.md` | One file per topic |
...
5. **7-day memory window**: Automatically injects the past week's experience context into the system prompt; experiences beyond the window are retrieved through search.
...
> This chapter's analysis is based on octos v0.1.0, with the octos-memory crate located at `crates/octos-memory/src/`. As of this writing, the hybrid search weight configuration and HNSW parameters have not changed significantly. The MemoryStore entity bank feature may be expanded in future versions.
