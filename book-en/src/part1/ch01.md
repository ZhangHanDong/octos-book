...
When you first open the octos code repository and see 130,000 lines of Rust, 287 source files, and 9 core crates organized in a workspace, a question inevitably arises: Why not Python? Aren't LangChain and AutoGen mature enough? Why not Go? Isn't its concurrency model simpler?
...
1. **Tool invocation is a core capability of Agents.** Agents don't just generate text -- they execute Shell commands, read and write files, and make network requests. Every tool invocation is a potential attack surface. octos has 14 built-in tools (file read/write, Shell execution, web search, etc.), each requiring an independent security policy.
...
- **Session-level serialization**: Messages from the same user must be processed in order; two messages cannot modify the same session state simultaneously. octos uses per-session `Mutex` to achieve this (`crates/octos-cli/src/serve/`).
- **Tool-level parallelism**: Within a single Agent iteration, multiple unrelated tool invocations should execute in parallel to reduce latency. octos uses `futures::future::join_all` for tool concurrency (`crates/octos-agent/src/turn.rs`).
- **Resource throttling**: Unlimited concurrency will exhaust system resources. octos limits the maximum number of concurrent sessions (default 10) via `tokio::sync::Semaphore`, preventing resource exhaustion.
- **Graceful shutdown**: When receiving SIGTERM, you cannot brutally kill ongoing Agent conversations. octos uses an `AtomicBool` flag to implement graceful shutdown (Release on store, Acquire on load), allowing in-progress conversations to finish naturally.
...
**Second, memory is a hard constraint for multi-tenancy.** Each Agent session needs to maintain conversation history, tool state, and context window. If each session uses 100MB of memory (not uncommon in Python applications), 10 concurrent sessions is 1GB, and 100 is 10GB. octos's core data structures are designed for zero-copy and minimal allocation -- for example, the `truncate_utf8` function (`crates/octos-core/src/utils.rs`) achieves safe truncation through UTF-8 character boundary detection, avoiding unnecessary string copies.
...
octos-llm's stateful SSE parser (`crates/octos-llm/src/sse.rs`) sets a 1MB buffer cap and uses an incremental parsing strategy -- data is appended to a pre-allocated buffer and scanned line by line rather than reallocated per event. This design avoids the cascading effect common in GC languages where "parsing triggers GC, GC blocks all connections."

**An often-overlooked cost: context compaction.** When conversation history approaches the LLM's context window limit (typically 128K-200K tokens), octos needs to perform Context Compaction -- summarizing old messages to free up space (see Chapter 8 for details). This operation involves heavy string processing and token counting, which in GC languages easily produces large amounts of temporary objects and GC pressure. octos minimizes memory overhead on these hot paths through zero-copy utility functions like `truncate_utf8` (`crates/octos-core/src/utils.rs`) and the allocation-free implementation of `estimate_json_size` (`crates/octos-agent/src/tools/`).
...
octos sets `unsafe_code = "deny"` in the workspace root `Cargo.toml` (`Cargo.toml:33`), meaning the entire project -- 9 core crates, 130,000 lines of code -- does not allow any `unsafe` code. This is not a lint suggestion but a compile-time hard constraint. Any code containing `unsafe` blocks cannot pass `cargo build`.
...
## 1.3 Workspace Topology: Layered Architecture of 9 Crates

octos uses a Cargo workspace to organize its code, with 9 core crates divided into four layers:
...
- **octos-core** (1,793 lines): Core type definitions -- `Task`, `Message`, `MessageRole`, `AgentId`, `SessionKey`, etc. This is the "domain language" of the entire system; all other crates share these type definitions. The zero-internal-dependency design ensures the stability of type definitions.
- **octos-plugin** (1,293 lines): Plugin SDK -- manifest.json parsing, plugin discovery (directory scanning + priority rules), triple-gate checks (binary/env/OS). Independent of the Agent runtime and usable standalone.
...
- **octos-llm** (15,728 lines): LLM Provider abstraction layer. Unifies the calling interfaces of Anthropic (Claude), OpenAI (GPT-4), Google Gemini, Ollama, and other Providers. Includes a three-tier fault-tolerance chain (RetryProvider -> ProviderChain -> AdaptiveRouter), SSE streaming parser, model catalog, and pricing calculation.
...
- **octos-bus** (19,634 lines): Message bus and channel integration. Supports 14 messaging channels including Telegram, Discord, Slack, WhatsApp, Feishu, Email, and more. Provides session management (JSONL persistence + LRU memory cache) and message chunking (5-level splitting strategy).

**Layer Two: Runtime Engine**

Depends on Layer Zero and Layer One, implementing core runtime logic:

- **octos-agent** (34,968 lines): Agent runtime -- the heart of the entire system. Contains the Agent main loop, tool registration and execution, command approval policies, sandbox integration, MCP client, Hook system, loop detection, context compaction, and more. Depends on octos-core, octos-llm, and octos-memory.
- **octos-pipeline** (about 9K lines): Workflow engine. Defines workflow topology using Graphviz DOT syntax. The current main path supports 6 `HandlerKind` variants: `Codergen`, `Shell`, `Gate`, `Noop`, `Parallel`, and `DynamicParallel`. `Parallel` / `DynamicParallel` are executor branches; `Gate` is a condition node, not the default human-approval path. Depends on octos-core, octos-agent, octos-llm, and octos-memory.

**Layer Three: User Entry Points**

- **octos-cli** (40,746 lines): CLI, Web, and MCP Server entry point -- the "front door" of the entire system. Provides four operating modes (`octos chat` interactive CLI, `octos gateway` message bus, `octos serve` Web Dashboard + REST/API/AppUI, and `octos mcp-serve` for outer orchestrators). Controls compilation of each channel integration (telegram, discord, slack, etc.) through feature flags. Depends on octos-core, octos-agent, octos-llm, octos-memory, octos-pipeline, and octos-bus.
...
graph BT
    subgraph "Layer Zero: Independent Infrastructure"
        core["octos-core<br/><i>1,793 lines · Core Types</i>"]
        plugin["octos-plugin<br/><i>1,293 lines · Plugin SDK</i>"]
        sandbox["octos-sandbox<br/><i>162 lines · Windows Sandbox</i>"]
    end

    subgraph "Layer One: Domain Services"
        llm["octos-llm<br/><i>15,728 lines · LLM Abstraction</i>"]
        memory["octos-memory<br/><i>1,748 lines · Hybrid Search</i>"]
        bus["octos-bus<br/><i>19,634 lines · Message Bus</i>"]
    end

    subgraph "Layer Two: Runtime Engine"
        agent["octos-agent<br/><i>34,968 lines · Agent Runtime</i>"]
        pipeline["octos-pipeline<br/><i>9,137 lines · Workflow Engine</i>"]
    end

    subgraph "Layer Three: User Entry Points"
        cli["octos-cli<br/><i>40,746 lines · CLI / Web / Gateway</i>"]
    end
...
**Figure 1-1: octos workspace dependency topology.** Arrow direction indicates "depends on," i.e., upper layers depend on lower layers. octos-cli has a hard dependency on octos-bus, but individual channel integrations within bus (Telegram, Discord, etc.) are enabled on demand through feature flags. Note that octos-plugin and octos-sandbox are independent -- they neither depend on nor are depended upon by the core crates. octos-agent directly contains plugin loading logic rather than depending on the octos-plugin crate.

### 1.3.3 Code Size Overview

| Crate | Lines of Code | Percentage | Core Responsibility |
|-------|--------------|------------|-------------------|
| octos-cli | 40,746 | 30.6% | Four operating modes + Web UI + REST API + MCP Serve |
| octos-agent | 34,968 | 26.3% | Agent main loop + Tool system + Security policies |
| octos-bus | 19,634 | 14.8% | 14 channel integrations + Session management |
| octos-llm | 15,728 | 11.8% | Multi-Provider abstraction + Fault tolerance + Streaming |
| octos-pipeline | 9,137 | 6.9% | DOT workflow engine |
| octos-core | 1,793 | 1.3% | Core type definitions |
| octos-memory | 1,748 | 1.3% | Embedded hybrid search |
| octos-plugin | 1,293 | 1.0% | Plugin discovery and gating |
| octos-sandbox | 162 | 0.1% | Windows sandbox |
| app-skills + platform-skills | ~7,961 | 6.0% | 10 skill binary programs |
| **Total** | **133,170** | **100%** | 9 core crates + 10 skill programs |

**Table 1-1: octos code size distribution.** Total of 133,170 lines of Rust code across 287 source files.

In addition to the 9 core crates, octos includes two categories of skill binary programs:

- **app-skills** (9): Application-level capabilities -- news aggregation, deep search, deep crawling, email sending, account management, time query, weather query, WeChat bridge, Pipeline approval. Each skill is a standalone binary program that interacts with the Agent through a stdin/stdout JSON protocol.
...
> - A single `cargo test --workspace` verifies compatibility across all 9 crates
...
>
> First, octos's 9 core crates are highly coupled -- octos-agent depends on octos-core, octos-llm, and octos-memory simultaneously. Any change to a core type ripples across multiple crates. In multi-repo mode, a field change to the `Message` type might require publishing 6 versions in order: core -> llm -> memory -> agent -> pipeline -> cli, with each version waiting for upstream publication before it can begin. In a workspace, this is a single commit.
>
> Second, `[workspace.dependencies]` (`Cargo.toml:36-50`) ensures all crates use exactly the same versions of tokio, serde, reqwest, and other critical dependencies, avoiding linking multiple versions of a runtime within the same program.
>
> Third, workspace-level `[workspace.lints.rust]` (`Cargo.toml:33`) makes the `deny(unsafe_code)` policy automatically cover all crates, without needing to repeat the declaration in each crate's `lib.rs`. This ensures consistency of the security policy -- no crate can accidentally omit this constraint.
...
3. **Workspace topology**: 9 core crates are organized into four layers -- Independent Infrastructure (core/plugin/sandbox) -> Domain Services (llm/memory/bus) -> Runtime Engine (agent/pipeline) -> User Entry Points (cli). Dependencies flow strictly from top to bottom, with channel integrations enabled on demand through feature flags.
...
> This chapter's analysis is based on octos v0.1.0 (workspace definition in `Cargo.toml`, edition = "2024", rust-version = "1.85.0"). As of the time of writing, the layered topology of the 9 core crates has not undergone any major changes.
