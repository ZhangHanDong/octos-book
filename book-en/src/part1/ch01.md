...
**Finally, upstream SSE stream parsing requires sustained CPU efficiency.** LLM provider streaming responses are transmitted in Server-Sent Events (SSE) format, and the framework needs to parse tokens within milliseconds of arrival before projecting them into Agent progress events. In multi-tenant scenarios, the platform may maintain dozens of upstream provider streams simultaneously, each lasting tens of seconds. If the parser triggers heap allocation for every event, allocation pressure under high concurrency leads to latency spikes.
...
- **octos-core** (8,757 lines): Core type definitions -- `Task`, `Message`, `MessageRole`, `AgentId`, `SessionKey`, etc. This is the "domain language" of the entire system; all other crates share these type definitions. The zero-internal-dependency design ensures the stability of type definitions.
- **octos-plugin** (2,882 lines): Plugin SDK -- manifest.json parsing, plugin discovery (directory scanning + priority rules), triple-gate checks (binary/env/OS). It is now depended on by octos-agent, separating plugin discovery and gating from the main loop.
- **octos-sandbox** (146 lines): Windows platform AppContainer sandbox helper. Minimal implementation, platform-specific.

**Layer One: Domain Services**

Depends on octos-core, providing domain-specific capabilities:

- **octos-llm** (17,604 lines): LLM Provider abstraction layer. Unifies the calling interfaces of Anthropic (Claude), OpenAI (GPT-4), Google Gemini, Ollama, and other Providers. Includes a three-tier fault-tolerance chain (RetryProvider -> ProviderChain -> AdaptiveRouter), credential pools, content classification, SSE streaming parser, model catalog, and pricing calculation.
- **octos-memory** (1,635 lines): Hybrid search memory system. Implements BM25 full-text search and HNSW vector indexing based on the redb embedded database, supporting Episode Store (task completion summaries with a 7-day window memory).
- **octos-bus** (30,270 lines): Message bus and channel integration. Supports messaging channels including Telegram, Discord, Slack, WhatsApp, Feishu, Email, and more. Provides session management, multi-tenant account binding, admin tokens, and message chunking strategies.

**Layer Two: Runtime Engine**

Depends on Layer Zero and Layer One, implementing core runtime logic:

- **octos-agent** (84,422 lines): Agent runtime -- the heart of the entire system. Contains the Agent main loop, tool registration and execution, command approval policies, sandbox integration, MCP client, Hook system, loop detection, context compaction, and more. Depends on octos-core, octos-llm, octos-memory, octos-bus, and octos-plugin.
- **octos-pipeline** (13,497 lines): Workflow engine. Defines workflow topology using Graphviz DOT syntax. The current main path supports handler kinds such as `Codergen`, `Shell`, `Gate`, `Noop`, `Parallel`, and `DynamicParallel`. Depends on octos-core, octos-agent, octos-llm, and octos-memory.
- **octos-swarm** (3,738 lines): Multi-sub-agent orchestration primitive. It persists swarm plans for fan-out, sequence, and pipeline patterns, using octos-agent underneath to run MCP-backed sub-agents.
- **octos-dora-mcp** (372 lines): Dora-RS to MCP tool bridge. It depends on octos-agent and wraps external dataflow/node capabilities as MCP tools callable by Agents.

**Layer Three: User Entry Points**

- **octos-cli** (89,237 lines): CLI, Web, and MCP Server entry point -- the "front door" of the entire system. Provides `chat`, `gateway`, `serve`, `mcp-serve`, and related modes, and carries the Web Dashboard, REST/API/AppUI, production control plane, and swarm command entry points. Controls compilation of each channel integration (telegram, discord, slack, etc.) through feature flags. Depends on octos-core, octos-agent, octos-llm, octos-memory, octos-pipeline, octos-bus, and octos-swarm.
...
graph BT
    subgraph "Layer Zero: Independent Infrastructure"
        core["octos-core<br/><i>8,757 lines · Core Types</i>"]
        plugin["octos-plugin<br/><i>2,882 lines · Plugin SDK</i>"]
        sandbox["octos-sandbox<br/><i>146 lines · Windows Sandbox</i>"]
    end

    subgraph "Layer One: Domain Services"
        llm["octos-llm<br/><i>17,604 lines · LLM Abstraction</i>"]
        memory["octos-memory<br/><i>1,635 lines · Hybrid Search</i>"]
        bus["octos-bus<br/><i>30,270 lines · Message Bus</i>"]
    end

    subgraph "Layer Two: Runtime Engine"
        agent["octos-agent<br/><i>84,422 lines · Agent Runtime</i>"]
        pipeline["octos-pipeline<br/><i>13,497 lines · Workflow Engine</i>"]
        swarm["octos-swarm<br/><i>3,738 lines · Sub-agent Orchestration</i>"]
        dora["octos-dora-mcp<br/><i>372 lines · Dora/MCP Bridge</i>"]
    end

    subgraph "Layer Three: User Entry Points"
        cli["octos-cli<br/><i>89,237 lines · CLI / Web / Gateway</i>"]
    end
...
|-------|--------------|------------|-------------------|
| octos-cli | 89,237 | 33.7% | Runtime modes + Web UI + REST API + MCP Serve + control plane |
| octos-agent | 84,422 | 31.9% | Agent main loop + Tool system + Security policies |
| octos-bus | 30,270 | 11.4% | Multi-channel integration + session/account management |
| octos-llm | 17,604 | 6.6% | Multi-Provider abstraction + Fault tolerance + Streaming |
| octos-pipeline | 13,497 | 5.1% | DOT workflow engine |
| octos-core | 8,757 | 3.3% | Core type definitions |
| octos-swarm | 3,738 | 1.4% | Sub-agent orchestration primitive |
| octos-plugin | 2,882 | 1.1% | Plugin discovery and gating |
| octos-memory | 1,635 | 0.6% | Embedded hybrid search |
| octos-dora-mcp | 372 | 0.1% | Dora-RS / MCP bridge |
| octos-sandbox | 146 | 0.1% | Windows sandbox |
| app-skills + platform-skills | 12,326 | 4.7% | 15 skill binary programs |
| **Total** | **264,886** | **100%** | 11 octos-* crates + 15 skill programs |

**Table 1-1: octos code size distribution.** Based on `tokei ../octos/crates --type Rust`, the current main branch contains 264,886 lines across 444 Rust files. Line counts are useful for scale, not as a quality metric; the current main branch is much larger than v0.1.0 because of the Web/API control plane, swarm, and harness starter skills.
