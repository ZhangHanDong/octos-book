...
- **octos-cli** (40,746 lines): CLI and Web entry point -- the "front door" of the entire system. Provides three operating modes (`octos chat` interactive CLI, `octos gateway` message bus, `octos serve` Web Dashboard + REST API). Controls compilation of each channel integration (telegram, discord, slack, etc.) through feature flags. Depends on octos-core, octos-agent, octos-llm, octos-memory, octos-pipeline, and octos-bus.
...
|-------|--------------|------------|-------------------|
| octos-cli | 40,746 | 30.6% | Three operating modes + Web UI + REST API |
