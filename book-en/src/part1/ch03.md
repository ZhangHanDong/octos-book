...
Four native protocols (Anthropic, OpenAI, Gemini, Ollama), while the remaining 10+ connect via OpenAI-compatible APIs (requiring only different base URLs and authentication methods). This "4 native + N compatible" architecture keeps the cost of onboarding new Providers extremely low—in most cases, only a single entry needs to be added to the registry.
