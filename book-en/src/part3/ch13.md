...
Serve provides the Web Dashboard, REST endpoints, SSE streaming output, and the UI Protocol WebSocket used by AppUI (`/api/ui-protocol/ws`). It is built with axum, and `AppState` holds global state such as providers, tool registries, and session management.

More precisely, Serve mode is now a control-plane aggregation point, not just "chat over HTTP". Startup composes Config / ProfileStore, LLM Provider / RetryProvider, ToolRegistry / ToolPolicy, SessionManager, REST/SSE/UI Protocol, and swarm dispatch state. The swarm state also projects `config.tool_policy` and injection-environment denylist into `DispatchPolicy::from_agent_gates(tool_policy, true)`, so swarm backends cannot bypass native tool policy (`crates/octos-cli/src/commands/serve.rs:1128-1156`).
...
4. **Serve control plane**: Serve aggregates REST, SSE, UI Protocol, profiles, tool policy, and swarm state; it is not merely a chat HTTP wrapper.
...
> This chapter's analysis is based on octos v0.1.0. The current main branch adds `octos mcp-serve` as a fourth entry point and fixes the Serve default port at 50080. The feature flag list may grow as functionality expands.
