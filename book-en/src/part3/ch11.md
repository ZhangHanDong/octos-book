...
> ### Engineering Decision Sidebar: Trade-offs of Per-session Mutex vs Actor Model
>
> The core advantage of the **Actor Model** is the message queue — each Actor has its own mailbox and processes messages in order. But octos's scenario has a key constraint: the Agent needs to return results to the caller after processing a message. In the Actor model, this requires an additional response channel and correlation ID, adding considerable complexity.
>
> **Per-session Mutex** directly expresses the semantics of "only one message is being processed at a time," and the code is simple and straightforward. Rust's `tokio::sync::Mutex` is async-aware — tasks waiting for the lock don't block the thread but instead yield execution to other tasks.
>
> The cost is: if Actor-level features are needed in the future (such as session-level mailbox queues, message priorities), a refactor to the Actor pattern would be required. But the current requirements (strict sequential processing + concurrency throttling) are well served by Mutex + Semaphore.
...
2. **Per-session Mutex**: Strict sequential processing within the same session, preventing state races.
...
5. **Graceful shutdown**: `AtomicBool` + Release/Acquire semantics, no loss of in-progress work.
...
> This chapter's analysis is based on octos v0.1.0. As of this writing, the core patterns of per-session Mutex and Semaphore rate limiting have not changed significantly. The default number of concurrent sessions may be automatically adjusted based on hardware in future versions.
