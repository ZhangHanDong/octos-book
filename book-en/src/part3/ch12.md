...
- Project `node_costs` into `ToolResult.structured_metadata` so the session actor can include per-node cost in the SSE `done` event (`../octos/crates/octos-pipeline/src/tool.rs:444-479`).
