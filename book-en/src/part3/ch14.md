...
3. **Observability and control**: Prometheus metrics, structured tracing, SSE event streams, and AppUI UI Protocol cover different operational needs.
4. **Multi-tenancy**: Profile/Account, User, and Session boundaries are separate. Sub-accounts inherit structured capability contracts but not customer-installed skills; process-level isolation depends on the process-manager path.
