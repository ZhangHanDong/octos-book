...
**Figure 14-3: Production control plane.** Serve aggregates setup wizard state, admin token rotation, SMTP configuration, profile configuration, and gateway child-process management. The actual message-processing gateways can be started and stopped per profile through process-manager.
...
> This chapter is based on the current `../octos` main branch. Compared with earlier versions, current main includes UI Protocol capability negotiation, `/api/events/harness` typed SSE, setup/admin token stores, SMTP secret store, structured sub-account inheritance, and process-manager per-profile gateways. Resource isolation still requires system-level limits.
